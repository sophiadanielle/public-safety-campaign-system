...
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'campaign_department_events' AND COLUMN_NAME = 'event_name');
...
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'campaign_department_events' AND COLUMN_NAME = 'linked_campaign_id');
SET @sql = IF(@col_exists = 0, 
...
ALTER TABLE `campaign_department_events` ADD COLUMN IF NOT EXISTS event_title VARCHAR(200) NULL AFTER id;
UPDATE `campaign_department_events` SET event_title = event_name WHERE event_title IS NULL AND event_name IS NOT NULL;

-- Update event_type enum to match requirements
ALTER TABLE `campaign_department_events` MODIFY COLUMN event_type ENUM('seminar', 'drill', 'workshop', 'orientation') NOT NULL DEFAULT 'seminar';
...
ALTER TABLE `campaign_department_events` 
    ...
    FOREIGN KEY (target_audience_profile_id) REFERENCES `campaign_department_audience_segments`(id) ON DELETE SET NULL;

-- Add foreign key for created_by
ALTER TABLE `campaign_department_events` 
    ADD CONSTRAINT IF NOT EXISTS fk_events_created_by 
    FOREIGN KEY (created_by) REFERENCES `campaign_department_users`(id) ON DELETE SET NULL;
...
CREATE TABLE IF NOT EXISTS `campaign_department_event_agency_coordination` (
    ...
    CONSTRAINT fk_event_agency_coordination_event FOREIGN KEY (event_id) REFERENCES `campaign_department_events`(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS `campaign_department_event_conflicts` (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_event_conflicts_event FOREIGN KEY (event_id) REFERENCES `campaign_department_events`(id) ON DELETE CASCADE,
    CONSTRAINT fk_event_conflicts_conflicting FOREIGN KEY (conflicting_event_id) REFERENCES `campaign_department_events`(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS `campaign_department_event_audit_log` (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_event_audit_event FOREIGN KEY (event_id) REFERENCES `campaign_department_events`(id) ON DELETE CASCADE,
    CONSTRAINT fk_event_audit_user FOREIGN KEY (user_id) REFERENCES `campaign_department_users`(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS `campaign_department_event_integration_checkpoints` (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_event_integration_event FOREIGN KEY (event_id) REFERENCES `campaign_department_events`(id) ON DELETE CASCADE,
...
UPDATE `campaign_department_events` SET event_status = 'draft' WHERE event_status IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_events_date_time_venue ON `campaign_department_events`(date, start_time, venue);
...
LEFT JOIN `campaign_department_event_facilitators` ef ON ef.event_id = e.id
LEFT JOIN `campaign_department_event_audience_segments` eas ON eas.event_id = e.id
LEFT JOIN `campaign_department_event_partners` ep ON ep.event_id = e.id
LEFT JOIN `campaign_department_event_agency_coordination` eac ON eac.event_id = e.id
GROUP BY e.id;
