...
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
    ...
    AND TABLE_NAME = 'audience_segments' 
    AND COLUMN_NAME = 'name');
    
SET @sql = IF(@col_exists > 0,
    'ALTER TABLE audience_segments CHANGE COLUMN name segment_name VARCHAR(150) NOT NULL',
    'SELECT 1');
...
SET @sql = IF(@col_exists = 0,
    'ALTER TABLE audience_segments ADD COLUMN location_reference VARCHAR(255) NULL AFTER geographic_scope',
    'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col_exists > 0,
    'ALTER TABLE audience_segments CHANGE COLUMN segmentation_basis basis_of_segmentation ENUM(\'Historical trend\', \'Inspection results\', \'Attendance records\', \'Incident pattern reference\') NULL',
    'ALTER TABLE audience_segments ADD COLUMN basis_of_segmentation ENUM(\'Historical trend\', \'Inspection results\', \'Attendance records\', \'Incident pattern reference\') NULL AFTER risk_level');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
LEFT JOIN audience_segments s ON s.id = am.segment_id
